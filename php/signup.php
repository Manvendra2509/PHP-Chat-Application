                    ...
                    if (in_array($img_ext, $extensions) === true) {
                        ...
                        if (in_array($img_type, $types) === true) {
                            ...
                            if (move_uploaded_file($tmp_name, "images/" . $new_img_name)) {
                                ...
                                if ($insert_query) {
                                    // If the user is successfully inserted, query the database again to retrieve the user data associated with the newly registered email
                                    $select_sql2 = mysqli_query($conn, "SELECT * FROM users WHERE email = '{$email}'");
                                    if (mysqli_num_rows($select_sql2) > 0) {
                                        // If the user data is successfully retrieved, start a PHP session and set the unique_id session variable to the user's unique ID
                                        $result = mysqli_fetch_assoc($select_sql2);
                                        $_SESSION['unique_id'] = $result['unique_id'];
                                        // Return "success" to the AJAX request
                                        echo "success";
                                    } else {
                                        // If the user data retrieval fails, return an error message
